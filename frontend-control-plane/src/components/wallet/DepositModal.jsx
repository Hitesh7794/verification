...
import { deposit, formatRupees } from '../../lib/wallet/wallet.js'
...
export default function DepositModal({ config, currentBalance, onClose, onSuccess }) {
  ...
  async function pay() {
    ...
    try {
      ...
      // Notify any other wallet UI on the page (header WalletWidget,
      // dashboard panel) to re-fetch immediately. Without this, the
      // header would lag by up to 4 seconds until its own poll fires.
      window.dispatchEvent(new CustomEvent('wallet:changed', {
        detail: { balance_paise: result.balance_paise },
      }))
      ...
      <Backdrop onClose={onClose}>
        <Modal onClose={onClose} title="Deposits unavailable">
          <p className="text-sm text-slate-600">
            Razorpay isn't configured on this deployment. Contact the
            platform team to enable online top-ups or request a manual
            credit from a superadmin.
          </p>
  ...
  // While Razorpay's hosted checkout is open, hide our own backdrop so
  // it doesn't half-cover the vendor's modal. We keep the component
  // mounted (just visually hidden) so that on error/cancel we can
  // re-show it with the entered amount and the error message intact.
  return (
    <Backdrop onClose={onClose} hidden={busy}>
      <Modal onClose={onClose} title="Add money to wallet">
        ...
        {/* Test-card hint shows ONLY when the deployment is using a Razorpay
            test key (rzp_test_*). In production (rzp_live_*) we hide it —
            otherwise operators would try the test card against live Razorpay
            and see a confusing "card declined" error. The backend's
            /api/wallet/config derives this from the key prefix. */}
        {config.razorpay_test_mode && (
...
// Backdrop renders to document.body via a portal. This is required
// because the AppShell header uses backdrop-blur, which creates a CSS
// containing block for `position: fixed` descendants. Without the
// portal, when the modal is opened from the WalletWidget (which lives
// inside the blurred header), `fixed inset-0` is computed relative to
// the navbar instead of the viewport — the modal ends up clipped and
// offset under the deposit button. Mounting into document.body lifts
// the modal out of all such containing blocks.
function Backdrop({ onClose, children, hidden = false }) {
