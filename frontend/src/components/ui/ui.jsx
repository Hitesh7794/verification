...
export function Button({ variant = 'primary', size = 'md', className = '', ...rest }) {
  ...
  const variants = {
    // Primary — deep espresso (near-black warm ink). Zero chromatic
    // clash with the warm parchment ground; amber does the only
    // coloured work in the palette (labels, brand mark, active states).
    primary:
...
export function Input({ className = '', ...rest }) {
  return (
    <input
      className={cn(
        'w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 placeholder-slate-400 focus:border-stone-700 focus:outline-none focus:ring-2 focus:ring-stone-300',
        className,
      )}
      {...rest}
    />
